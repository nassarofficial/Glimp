...
#define kMaxCharacterCount 180
#define kFontName @"[z] Arista"
#define kPlaceholderText @"Leave your Comment..."
...
-(void)setupView {
	...
	[messageTextView setFont:[UIFont systemFontOfSize:15.0f]];
	...
	[characterCountLabel setFont:[UIFont systemFontOfSize:10.0f]];
...
-(void)textViewDidEndEditing:(UITextView *)textView {
	...
	if([[textView text] length] == 0 && !showingPlaceholder) {
        ...
        if(delegate && [delegate respondsToSelector:@selector(statusTextView:postedMessage:)]) {
            [delegate statusTextView:self postedMessage:@""];
        }
    }else{
        if(delegate && [delegate respondsToSelector:@selector(statusTextView:postedMessage:)]) {
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
	...
	if ([text isEqualToString: @"\n"]||  textView.text.length + (text.length - range.length) > kMaxCharacterCount) {
		// Hide the keyboard
