...
class LocationCollectionViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if locid == "" {
            ...
            Alamofire.request(.GET, "https://api.foursquare.com/v2/venues/\(locid)?client_id=DPUDNKBQKC12FJ0KBRLVU5BG4JREZV1YAIVRXIULPLFZHMAL&client_secret=PA0I5FS3JWPDMOL0H5XLQNDJYBJPGNOEP4QX1ML23YOFGZ3G&v=20140806")
                .responseJSON { response in
                    if let json = response.result.value {
                        var json = JSON(json)
                        ...
                        let a1 = json["response"]["venue"]["location"]["city"].string
                        let a2 = json["response"]["venue"]["location"]["country"].string
                        ...
                        {
                            let properurl = prefix! + ims + suffix!
