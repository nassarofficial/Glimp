...
-(id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    ...
    if (self) {
        ...
        NSArray *array = [[NSArray alloc] initWithObjects:self, navg, notifications, nil];
...
- (void)addGestureRecognizers {
    ...
    UITapGestureRecognizer *closePinView = [[UITapGestureRecognizer alloc] initWithTarget: self action:@selector(closePOIActionsWindow)] ;
    closePinView.numberOfTapsRequired = 1;
    [self.view addGestureRecognizer:closePinView];
}
...
-(void)viewDidLoad {
    ...
    if (!self.fromSearch) {
        self.getFriends?[self getFriednsGlimps:nil]:[self getvideos];
...
-(void)enableFlash{
    videoRecorder.cameraFlashMode = UIImagePickerControllerCameraFlashModeOn;
        [self.flashButton setImage:[UIImage imageNamed:@"flash-selected"] forState:UIControlStateNormal];
        [self.flashButton setTitle:nil forState:UIControlStateNormal];
