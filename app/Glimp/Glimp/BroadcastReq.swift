...
class BroadcastReq: UIViewController,MKMapViewDelegate, CLLocationManagerDelegate {
    ...
    Alamofire.request(.POST, "http://glimpglobe.com/v2/broadcastreq.php", parameters: ["b_id": self.broadcast_id, "lat": gllatitude, "lon": gllongitude, "secid": "yMPxQSTXpUC7gB8uK4h9v9fUeYNsPjnPzw4dcR3y"]).response { request, response, json, error in
        if json != nil {
        ...
        let ladelta : CLLocationDegrees = 0.001
        let lndelta : CLLocationDegrees = 0.001
        let latitude2 = self.latitude
        let longitude2 = self.longitude
        
        let s:MKCoordinateSpan = MKCoordinateSpanMake(ladelta,lndelta)
        let l:CLLocationCoordinate2D = CLLocationCoordinate2DMake(latitude2, longitude2)
...
//        self.gllongitude = Float(userLocation.coordinate.longitude)
        ...
        let height = 50000.0
        _ = MKCoordinateRegionMakeWithDistance(center, width, height)
    }
    ...
    func locationManager(manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        //1 grab current location        let locValue:CLLocationCoordinate2D = locations.last!

        let currentLocation = locations.last
        //2 create location point for map based on current location
        _ = CLLocationCoordinate2D(latitude: currentLocation!.coordinate.latitude, longitude: currentLocation!.coordinate.longitude)
        ...
        Alamofire.request(.GET, "http://glimpglobe.com/v2/distance.php", parameters: ["b_id": self.broadcast_id, "lat": self.gllatitude, "lon": self.gllongitude]).response { request, response, json, error in
            if json != nil {
                var jsonObj = JSON(json!)
                print(request)
